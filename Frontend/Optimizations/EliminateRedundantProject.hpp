...
#ifndef FRONTEND_ELIMINATEREDUNDANTPROJECT_HPP
#define FRONTEND_ELIMINATEREDUNDANTPROJECT_HPP
...
class EliminateRedundantProject : public PlanOptimizer
{
    ...
    public:
        ...
        {
            return new ConstantExpression(node->getSourceLocation(),node->getValue(),node->getType()->getType());
        }
...
public:
    ...
    public:
        ...
        void *VisitFilterNode(FilterNode *node, void *context) override {
            ...
            return this->planNodeAllocator->record(newNode->replaceChildren({rewrittenNode}));
        }
        ...
        void *VisitFinalAggregationNode(FinalAggregationNode *node, void *context) override {
            ...
            auto  newNode = this->planNodeAllocator->new_FinalAggregationNode(node->getId(),node->getSource(),newAggregations,newGroupByKeys);
        ...
        void *VisitPartialAggregationNode(PartialAggregationNode *node, void *context) override {
            ...
            auto  newNode = this->planNodeAllocator->new_PartialAggregationNode(node->getId(),node->getSource(),newAggregations,newGroupByKeys);
        ...
        void *VisitProjectNode(ProjectNode *node, void *context) override {
            ...
            bool isOutputProject = false;
            if(this->getParent() != NULL && this->getParent()->getType() == "TaskOutputNode")
                isOutputProject = true;
            ...
            for(auto assignment : assignments->getMap())
            {
                ...
                RowExpression *rowExpressionRewrittenLeft = assignment.first.get();
                rowExpressionRewrittenLeft = rewriteContext->rewrite(assignment.first.get(),context);
            ...
            auto newNode = planNodeAllocator->new_ProjectNode(node->getId(),rewrittenNode,assignmentsBuilder->build());
...
#endif //FRONTEND_ELIMINATEREDUNDANTPROJECT_HPP
