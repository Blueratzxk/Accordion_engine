...
#include "./DBGenIterator.h"

#include "./dbgen/include/dbgen/dbgen_gunk.hpp"
...
static std::mutex DBGenBackendSingletonLock;
static std::shared_ptr<DBGenBackend> DBGenBackendSingleton = NULL;
...
DBGenIterator::DBGenIterator(double scaleFactor) {
    ...
    std::shared_ptr<DBGenBackend> dbgenBackend;
    DBGenBackendSingletonLock.lock();
    if(DBGenBackendSingleton == NULL) {
        DBGenBackendSingleton = std::make_shared<DBGenBackend>();
        dbgenBackend = DBGenBackendSingleton;
    }
    else
        dbgenBackend = DBGenBackendSingleton;
    DBGenBackendSingletonLock.unlock();
