...
#define OLVP_QUERY1_SSINGLE_HPP
...
class Query1_ssingle:public RegQuery
{
...
public:
    ...
    PlanNode* getPlanTree()
    {
        ...
        tableScanLocalExchange->addSource(tableScanLineItem);
        ...
        LocalExchangeNode *local = new LocalExchangeNode("local");
        local->addSource(partialAgg);

        PlanNode *finalAgg = createFinalAgg();
        finalAgg->addSource(local);
