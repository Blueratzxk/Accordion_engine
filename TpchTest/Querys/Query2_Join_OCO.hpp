//
// Created by zxk on 12/19/25.
//

#ifndef OLVP_QUERY2_JOIN_OCO_HPP
#define OLVP_QUERY2_JOIN_OCO_HPP
...
/*select count(o_orderkey) from orders,customer where orders.o_custkey=customer.c_custkey;*/

class Query2_Join_OCO:public RegQuery {
...
public:
    ...
    PlanNode *getPlanTree() {
        ...
        shared_ptr<PartitioningScheme> scheme2 = make_shared<PartitioningScheme>(
                Partitioning::create(SystemPartitioningHandle::get("SINGLE_DISTRIBUTION"), {}));
    ...
    LookupJoinNode *createOrdersJoinCustomer() {
        ...
        LookupJoinDescriptor RNLookupJoinDescriptor(ordersProbeSchema, ordersprobeHashChannels,
                                                    ordersprobeOutputChannels, customerBuildSchema,
                                                    customerbuildHashChannels, customerbuildOutputChannels,
                                                    customerBuildOutputSchema);
        ...
        shared_ptr<PartitioningScheme> schemeBuild = make_shared<PartitioningScheme>(
                Partitioning::create(SystemPartitioningHandle::get("SCALED_HASH_REDISTRIBUTION_BUF"), {}), outputLayout,
                hashColumnBuild, bucketToPartition);
        ExchangeNode *buildExchange = new ExchangeNode("buildExchange", ExchangeNode::REPLICATE, schemeBuild,
                                                       tableScanCustomer);

        shared_ptr<PartitioningScheme> schemeProbe = make_shared<PartitioningScheme>(
                Partitioning::create(SystemPartitioningHandle::get("SCALED_HASH_DISTRIBUTION_BUF"), {}), outputLayout,
                hashColumnProbe, bucketToPartition);
        ExchangeNode *probeExchange = new ExchangeNode("probeExchange", ExchangeNode::REPARTITION, schemeProbe,
                                                       tableScanOrders);
    ...
    PlanNode *createCustomerTableScan() {
        ...
        projectNode->addSource(tableScanCustomer);
    ...
    PartialAggregationNode *createPartialAgg() {
        ...
        AggregationDesc PartialAggDesc(
                {AggregateDesc(/*functionName=*/"count_all",/*inputKey=*/"",/*outputName=*/"pcount_orderkey")},
                /*groupByKeys=*/{});

        PartialAggregationNode *partialAggregationNode = new PartialAggregationNode(UUID::create_uuid(),
                                                                                    PartialAggDesc);
    ...
    FinalAggregationNode *createFinalAgg() {
        ...
        AggregationDesc FinalAggDesc(
                {AggregateDesc(/*functionName=*/"sum",/*inputKey=*/"pcount_orderkey",/*outputName=*/"fcount_orderkey")},
                /*groupByKeys=*/{});
...
#endif //OLVP_QUERY2_JOIN_OCO_HPP
