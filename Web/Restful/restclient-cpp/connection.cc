...
RestClient::Connection::Connection(const std::string& baseUrl,bool isLong)
        : headerFields(), lastRequest() {
    this->curlHandle = curl_easy_init();

    curl_easy_setopt(curlHandle, CURLOPT_TCP_KEEPALIVE, 1L);
    curl_easy_setopt(curlHandle, CURLOPT_TCP_KEEPIDLE, 300L);
    curl_easy_setopt(curlHandle, CURLOPT_TCP_KEEPINTVL, 20L);
...
static int debug_callback(CURL *handle, curl_infotype type, char *data, size_t size, void *userdata) {
    FILE *logfile = (FILE *)userdata;
    if (logfile) {
        fwrite(data, 1, size, logfile);
        fflush(logfile); // 确保实时写入
    }
    return 0;
}
...
RestClient::Connection::performCurlRequest(const std::string& uri,
    ...
    {
        if(debugFile == NULL)
            debugFile = fopen("curlDebug.log", "a+");
        curl_easy_setopt(getCurlHandle(), CURLOPT_VERBOSE, 1L);

        // 设置调试回调函数
        curl_easy_setopt(getCurlHandle(), CURLOPT_DEBUGFUNCTION, debug_callback);
        curl_easy_setopt(getCurlHandle(), CURLOPT_DEBUGDATA, debugFile);
        curl_easy_setopt(getCurlHandle(), CURLOPT_STDERR,debugFile);
    }
  ...
  if(this->timeoutMS)
  {
      ...
      curl_easy_setopt(getCurlHandle(), CURLOPT_TIMEOUT_MS, this->timeout);
      curl_easy_setopt(getCurlHandle(), CURLOPT_CONNECTTIMEOUT, 1);

      // dont want to get a sig alarm on timeout
      curl_easy_setopt(getCurlHandle(), CURLOPT_NOSIGNAL, 1);

      //curl_easy_setopt(getCurlHandle(), CURLOPT_CONNECTTIMEOUT_MS, this->timeoutMS);
      //curl_easy_setopt(getCurlHandle(), CURLOPT_TIMEOUT_MS, this->timeoutMS);
  }
...
void RestClient::Connection::setDebug() {
    this->debug = true;
}

void RestClient::Connection::SetTimeoutMS(int ms) {
    this->timeoutMS = ms;
}
